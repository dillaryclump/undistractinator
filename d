...
def is_coding(focus_apps):
    """Check if a focus app is the frontmost application."""
    try:
        ...
        for app in focus_apps:
            if app.lower() in active_process.lower():
                logging.debug(f"Detected focus app in front: {active_process}")
                return True
    except subprocess.CalledProcessError as e:
...
def main():
    ...
    focus_apps = config.get("focus_apps", DEFAULT_CODE_EDITORS)
